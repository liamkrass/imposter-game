...
export function getRandomWord(allowedCategories = [], customWords = []) {
    let availableCategories = [...wordList];

    // Add Custom category if valid
    ...
    if (allowedCategories && allowedCategories.length > 0) {
        availableCategories = availableCategories.filter(c => allowedCategories.includes(c.category));
    }

    if (availableCategories.length === 0) {
        // Fallback: If Custom was the only one and it filtered out everything else (e.g. empty custom list), 
        // OR if user deselected everything. 
        // Reset to full wordList is probably safer, or error.
        availableCategories = wordList;
    }

    const categoryObj = availableCategories[Math.floor(Math.random() * availableCategories.length)];
